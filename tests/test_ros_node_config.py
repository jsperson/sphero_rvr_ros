...
def test_default_node_config_uses_pi_serial_alias_and_floor_turn_motor_duty():
    ...
    assert config.pivot_target_rate_rad_s == 1.3
    assert config.pivot_max_duty == 32
    assert config.pivot_min_duty == 23
...
def test_create_driver_passes_base_driver_safety_limits():
    ...
    assert driver._pivot_target_rate_rad_s == 0.9
    assert driver._pivot_max_duty == 33
