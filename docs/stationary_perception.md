...
The stationary perception runtime runs live Pi lidar, `slam_toolbox`, and camera perception while the
...
Because this runtime is immobile, the launch publishes a fixed identity
...
estimation before any moving mission.
...
libcamera device path. Stationary perception therefore uses a runtime-only copy of the
...
`rvr-telemetry.service` is installed but not automatically enabled
...
running stationary mission to terminate on freshness. Restarting sensors only
creates new live evidence; it cannot restore an expired mission intent.

The web console may explicitly start or stop this one fixed user unit
