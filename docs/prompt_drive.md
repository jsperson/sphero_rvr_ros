...
proposal using the first-party OpenAI Responses API. Proposal-only is the
default: it does not import ROS, publish a route, or move the rover.

## No-motion proposal

Set `OPENAI_API_KEY` in the runtime environment, then run:
...
Use `--model` or `--reasoning-effort` to select another supported first-party
configuration. The generated manifest is written under
